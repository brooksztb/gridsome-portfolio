        ...
        >
          <div class="w-full md:h-full h-screen-1/3 overflow-hidden bg-white">
            <g-image
              ...
              :src="imageUrl(post.node.hero_image)"
              :alt="post.node.title"
              width="300"
              height="300"
              quality="75"
          ...
          >
            <h2
              class="mb-2 md:translate-x-px10 translate-x-0 transition-transform duration-500 ease-out"
            >{{ post.node.title }}</h2>
            <h3
              class="mb-4 xl:mb-5 md:translate-x-px10 translate-x-0 transition-transform duration-500 ease-out"
            >{{ formatDate(post.node.date) }}</h3>
            <p
              class="max-w-px900 text-gray-700 md:translate-x-px10 translate-x-0 transition-transform duration-500 ease-out"
...
export default {
  ...
  methods: {
    ...
    imageUrl(base) {
      return `${this.$static.metadata.cloudinaryUrl}/w_1000,q_75/${base}`;
    }
  }
};
</script>
